...
В случае корректности введенных данных, программа выведет сообщение об успешной замене и закроется через 1 секунду и запустится Dota, в противном случае будет выведено сообщение об ошибке и закроется через 5 секунд.

Для автоматической замены и запуска необходимо добавить параметр distance в settings.ini, если необходимо выбирать разные значения при каждом запуске, то distance должно быть 0.
